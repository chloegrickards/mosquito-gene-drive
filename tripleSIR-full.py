...
# Verify in literature, or get from model
beta = 0.1
#
...
for i in range(1,ngen):
   ...
   # print(population_curr)
    ...
    # dynamics of the proportion of humans that are infected
    #dXhdt = ((m*(a**2)*b*c*np.exp(-g*n))/(g + a*c*Xh))*(Xh * (1-Xh)) - r*X
    #Ih[i] =
...
pl.plot(tspan, Eg, 'b-', label='Eg')
pl.plot(tspan, Ig, 'g-', label='Ig')
